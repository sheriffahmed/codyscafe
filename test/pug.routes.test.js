...
describe('Pug model', () => {
  ...
  describe('column definitions and validations', () => {
    ...
    it('has a one-many relationship with Coffee, via `favoriteCoffee`', async () => {
      const [pug, createdCoffee] = await Promise.all([
        Pug.create({ name: 'Joe' }),
        Coffee.create({
          name: 'Puppaccino',
      ...
      await pug.setFavoriteCoffee(createdCoffee);
      expect(pug.favoriteCoffeeId).to.be.equal(createdCoffee.id);
    });

    // Make sure that you define the associations in `server/models/index.js`!
    // Note: be careful - the pluralization is important here!
    it('has a many-many relationship with other Pugs as `friends`', async () => {
      const [penny, doug] = await Promise.all([
  ...
  describe('instance method: isPuppy', () => {
    it('returns true if a pug is a puppy (less than one year old)', async () => {
      const [pup, notPup] = await Promise.all([
        Pug.create({ name: 'Pupster', age: 0 }),
        Pug.create({ name: 'Grouchy', age: 2 })
      ]);
  ...
  describe('class method: `findByCoffee`', () => {
    it('finds all pugs with the given favorite coffee', async () => {
      const [latte, cortado] = await Promise.all([
        Coffee.create({ name: 'latte' }),
        Coffee.create({ name: 'cortado' })
      ]);
      await Promise.all([
      ...
      const [latteLovers, cortadoCourtiers] = await Promise.all([
        Pug.findByCoffee('latte'),
        Pug.findByCoffee('cortado')
      ]);
  ...
  describe('hooks', () => {
    it("capitalizes a pug's name before saving it to the database", async () => {
      ...
      await pug.update({ name: 'joey' });
      expect(pug.name).to.equal('Joey');
    });
  });
});
